...
- 当前版本为 `3.0.9`，Android `versionCode 85`；本轮先发 Web（Cloudflare 跟 main），正式 APK 路径待打包为 `dist/coffee-vault-3.0.9-release.apk`。
...
- “文本联想输入”用于店铺名等可重复但允许自由输入的短文本字段：空输入时优先展示最近记录，输入后按精确、前缀、包含、子序列匹配并以最近使用时间排序；忽略大小写去重，最多展示 4 条。候选项复用轻量胶囊样式，点击只回填不自动提交；数据仅来自本地现有记录，展示前需过滤已删除项并安全转义。
