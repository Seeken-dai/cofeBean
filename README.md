...
当前版本：2.1.2（Android versionCode 45）
...
- `dist/coffee-vault-2.1.2-release.apk`
...
- 新用户空豆仓引导、喝一杯发现提示和进阶功能轻提示；右下浮动按钮空闲后自动收起，减少遮挡列表内容
- 2.0.9 完成 UI 视觉优化收尾：赏味期彩色标签、处理法角标、近 30 天饮用趋势、拍照入口轻推、tab 切换动效和新增豆子克重自动带出
- 2.1.0 优化冲煮辅助：圆环突出本段注水量、常驻下一段预览、支持手动结束记录用时、辅助按钮加大、入口移到底部；并修复联网时 iOS PWA 黑屏/白屏与安卓启动系统栏发白
- 2.1.1 高级评价多维雷达图（苦度反向、进入动效，详情与列表同步）；冲煮辅助圆环更顺滑、点圆环进下一段、两段间等待倒计时回退；首次喝一杯自动记开封日期、水温从方案正确带入
- 2.1.2 退出改为轻提示（再次返回即退出）；高级评价每维加「?」说明浮窗（编辑与设置）；列表雷达图移到卡片右侧并加单字标签；修复冲煮辅助圆环段末/间奏跳切与每次更新后 Web 打开偶发报错（SW 改为按版本整套预缓存）
...
- 咖啡豆分享可选择是否加入咖啡袋和标签图片；冲煮方案分享可选择是否加入离线导入二维码，或直接复制 `DC1-` 分享码；Android 真机保存分享 PNG 时写入系统相册 `Pictures/豆仓分享卡`，分享时写入缓存目录并调用系统分享面板，Web 预览则下载 PNG。
...
当前包信息：`com.coffeebean.vault`，版本 `2.1.2`（versionCode `45`），最低 Android 7/API 24，目标 Android 16/API 36。
