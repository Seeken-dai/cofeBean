...
3.0.2 正式包从 `main` 的 3.0.2 发布提交干净构建（Microsoft JDK 21.0.11 + `cap sync` + `clean assembleRelease`），`versionName=3.0.2`、`versionCode=79`、`minSdk 24` / `targetSdk 36`；权限仍为 `CAMERA`、`INTERNET`、`VIBRATE` 及 Android 自动生成的应用内接收器权限，未新增权限。签名证书与既有正式版一致，可覆盖升级并保留数据库。本版重构移动端与宽屏个人页交互：移动端「我的」采用纯净功能导航中心、子功能全部直达独立 Sheet 新页面；PC 宽屏回顾看板重构为两列双行排版，彻底修复近 6 个月饮用趋势条形图高度拉伸与底部空隙问题；**不修改 SQLite、备份、同步协议、Worker 或 Android 权限**，`user_version` 仍为 12。发布前 301 项 Node 测试、ESLint、`app.js` / `repository.js` 语法检查、Capacitor 同步和 Release 构建全部通过。正式产物为 `dist/coffee-vault-3.0.2-release.apk`，APK SHA-256 为 `41725348AA48FC6E2983BC4DB75580A69CECDF4354D998CB812508B761910438`。

3.0.1 正式包从 `main` 的 3.0.1 发布提交干净构建（Microsoft JDK 21.0.11 + `cap sync` + `clean assembleRelease`），已用 `aapt2 dump badging` 核对 `versionName=3.0.1`、`versionCode=78`、`minSdk 24` / `targetSdk 36`；权限仍为 `CAMERA`、`INTERNET`、`VIBRATE` 及 Android 自动生成的应用内接收器权限，未新增权限。`apksigner verify --print-certs` 核对证书 SHA-256 为 `aab5e3d3bd224b98f885945ecd868d54a99e2c96bf099a0c9e6ee59ca02151ae`，与既有正式版一致，可覆盖升级并保留数据库。本版收口宽屏“我的”内嵌子页、回顾趋势/评分/最近一杯卡片、个人页搜索对齐、报告详情返回路径和设置/侧栏滚动问题；**不修改 SQLite、备份、同步协议、Worker 或 Android 权限**，`user_version` 仍为 12。发布前 301 项 Node 测试、ESLint、`app.js` / `repository.js` 语法检查、Capacitor 同步和 Release 构建全部通过；本机未连接 ADB 真机，未执行安装后的可视点击冒烟。正式产物为 `dist/coffee-vault-3.0.1-release.apk`，APK SHA-256 为 `3dd504b78ddf5ee2d6d53f20cd80dd4febd11ccd492af6491da5e7b3976af74a`。

3.0.0 正式包从 `main` 发布提交 `120e335` 干净构建（`cap sync` + `clean assembleRelease`），已用 `aapt dump badging` 核对 `versionName=3.0.0`、`versionCode=77`、`minSdk 24` / `targetSdk 36`；权限与 2.4.3 逐项一致，仍为 `CAMERA`、`INTERNET`、既有 Haptics 引入的 `VIBRATE` 及 Android 自动生成的应用内接收器权限，未新增权限。`apksigner verify --print-certs` 核对证书 SHA-256 为 `aab5e3d3bd224b98f885945ecd868d54a99e2c96bf099a0c9e6ee59ca02151ae`，与既有正式版一致，可覆盖升级并保留数据库；APK 内 `index.html`、`styles.css`、`app.js` 的 SHA-256 也与当前源码逐项一致。本版完成全应用视觉与导航改版，并根据两轮 Android debug 包真机反馈修复底部弹窗定位、个人页宽度与重复入口、详情页置顶工具栏、列表标题对齐、文字裁切、头图遮挡和饮用卡片辅助文字底色等问题；**不修改 SQLite、备份、同步协议、Worker 或 Android 权限**，`user_version` 仍为 12。发布前 299 项 Node 测试、ESLint、`app.js` / `repository.js` 语法检查、Web 移动端/宽屏检查和 Release 构建全部通过，用户完成 debug 包真机验收并确认收尾发布。正式产物为 `dist/coffee-vault-3.0.0-release.apk`，APK SHA-256 为 `42197bf1aa650cd2ad75053107c0dbee6b20ad79d34849f59554c93fb89916f8`。

2.4.3 正式包从 `main` 发布提交 `eebcefc` 干净构建（`cap sync` + `clean assembleRelease`；首次命令因工具等待超时但 Gradle 继续完成，随后 `assembleRelease` 明确返回成功），已用 `aapt2 dump badging` 核对 `versionName=2.4.3`、`versionCode=76`、`minSdk 24` / `targetSdk 36`；`aapt2 dump permissions` 与 2.4.2 逐项一致，仍为 `CAMERA`、`INTERNET`、既有 Haptics 引入的 `VIBRATE` 及 Android 自动生成的应用内接收器权限，未新增权限。`apksigner verify --print-certs` 核对证书 SHA-256 为 `aab5e3d3bd224b98f885945ecd868d54a99e2c96bf099a0c9e6ee59ca02151ae`，与既有正式版一致，可覆盖升级并保留数据库。本版修复外饮表单填完后调起相机，Android 因内存压力回收 App 进程，确认照片返回时 WebView 重建并丢失未保存内容的问题：拍照前临时保护外饮字段、评分与照片草稿，监听 Capacitor `appRestoredResult` 后恢复弹窗和 Camera 结果；**不修改 SQLite、备份、同步协议、Worker 或 Android 权限**，`user_version` 仍为 12。发布前 290 项 Node 测试、ESLint、`app.js` / `repository.js` 语法检查和 Release 构建全部通过；用户明确要求直接发布，未在真机上复现系统回收并做覆盖安装验收。正式产物为 `dist/coffee-vault-2.4.3-release.apk`，APK SHA-256 为 `41cfa069167b08aa0ecf0af07a144dddb6f6f8c6454f65bf5fb884bb8eaab8a4`。
