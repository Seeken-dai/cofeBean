...
- ❌ **验证 3 图片同步：发现存量图片 bug（P0，未修）**。
  ...
  - **已彻底闭环（真机跨端复验通过）**：映夏、金菠萝等所有存量图在 web 端均正常显示。金菠萝暴露出补传的两处深层缺陷，均已修（`c7f252e`）：
    1. LWW 拒绝：补传的 r2 记录 updatedAt/revision 与云端脏记录相同 → 服务端判“非更新”丢弃。beanIdbToR2 转换成功时 bump revision。
    2. 缓存中毒：上一版把“blob 上传成功”当“云端已同步”，push 被 LWW 拒时二者分叉，缓存命中再不补推。改为只有 pull 回显 r2 才算确认；持久字段 imageRefs→imageSynced，丢弃旧中毒缓存。
  - 新需求已实现并真机验证：编辑器袋子/标签图片支持“删除”（`www/app.js` `removeBeanImage` + `imageCard` 删除按钮 + `www/styles.css`），随同步传播删除。已提交 `0b87ec8`/`8770d5e`/`c7f252e`。
  - 验证 4 已真机确认：同步失败时“最近失败 时间：原因”文案在账号卡片内两行自然换行、不溢出。

## 全部四项真机验证结论：通过

1. ✅ 覆盖升级数据保留　2. ✅ 登录同步　3. ✅ 图片上行/下行 + 存量图修复　4. ✅ 小屏错误文案换行。
docs/CHANGELOG.md 已补 2.0.6 条目。测试 99/99。剩余：是否推远端/出 dist 发布产物由用户决定（未经同意不推、不改 dist）。
