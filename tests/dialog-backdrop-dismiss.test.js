...
test('编辑类弹窗与冲煮辅助完全不参与点背景关闭', () => {
  const listStart = appSource.indexOf('[els.backup, els.calendar');
  const listEnd = appSource.indexOf('.forEach((dialog) => {', listStart);
  const bindings = appSource.slice(listEnd, appSource.indexOf("$('#photoSourceClose')", listEnd));
  const list = appSource.slice(listStart, listEnd);
