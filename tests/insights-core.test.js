...
test('手冲回顾首页归入第04类，单豆页面按层级返回', () => {
  ...
  // 手冲仍是首页子页返回“回顾首页”；月报/图鉴已移出首页，返回改为“个人中心”，日历直达的月报详情返回为“返回”。
  assert.match(source, /isBeanReview \? '返回手冲回顾'/);
  assert.match(source, /backToPersonal \? '返回个人中心'/);
  assert.match(source, /'返回回顾首页'/);
...
test('咖啡月报与年报移出回顾首页，来自列表回列表、否则退出回顾', () => {
  const source = fs.readFileSync(path.join(__dirname, '..', 'www', 'app-insights.js'), 'utf8');
  const html = fs.readFileSync(path.join(__dirname, '..', 'www', 'index.html'), 'utf8');
  ...
  // 报告详情：来自列表则返回列表，否则退出回顾。
  assert.match(source, /if \(state\.insightsReportFromList\)[\s\S]*?state\.insightsPage = 'reports'/);
...
test('豆款收集墙按同名同烘焙商合并复购：杯数累加、记复购次数、封面取最近有图的一次', () => {
  const beans = [
    bean('r1', { name: '花魁 7.0', roaster: 'Mock 烘焙', openedDate: atLocal(2026, 5, 1), bagImagePath: 'old.jpg' }),
    bean('r2', { name: '花魁 7.0', roaster: 'Mock 烘焙', openedDate: atLocal(2026, 6, 1), bagImagePath: 'new.jpg' }),
    bean('r3', { name: '花魁 7.0', roaster: '另一家', bagImagePath: 'other.jpg' }),
    bean('k1', { name: '肯尼亚', roaster: '山谷' })
  ];
  const rows = [
    log('l1', atLocal(2026, 5, 10), { beanId: 'r1' }),
    log('l2', atLocal(2026, 5, 12), { beanId: 'r1' }),
    log('l3', atLocal(2026, 6, 5), { beanId: 'r2' }),
    log('l4', atLocal(2026, 6, 11), { beanId: 'k1' })
  ];
  const result = insights.coffeeCatalog(rows, beans, { now: new Date(2026, 6, 15) });
  assert.equal(result.ok, true);
  // 同名不同烘焙商不合并：花魁(Mock)、花魁(另一家)、肯尼亚 => 3 格
  assert.equal(result.data.wall.length, 3);
  const merged = result.data.wall.find((item) => item.name === '花魁 7.0' && item.purchaseCount === 2);
  assert.ok(merged, '同名同烘焙商应合并为一格');
  assert.equal(merged.cups, 3);
  assert.equal(merged.cover.candidates[0].path, 'new.jpg');
  const single = result.data.wall.find((item) => item.name === '花魁 7.0' && item.purchaseCount === 1);
  assert.equal(single.cover.candidates[0].path, 'other.jpg');
  assert.equal(result.data.wall.find((item) => item.name === '肯尼亚').purchaseCount, 1);
});
...
test('冲煮里程碑只计自家冲煮，外饮不再混入；同日多杯只算一个连续日并排除无效记录', () => {
  const now = new Date(2026, 6, 15, 18);
  const rows = [];
  ...
  // 8 杯自家冲煮；两杯外饮归外饮图鉴，不再冲高这里的杯数。
  assert.deepEqual(milestones.cups, { value: 8, achieved: null, next: 10, remaining: 2, complete: false });
...
test('外饮不能替自家冲煮续上连续天数', () => {
  const now = new Date(2026, 6, 15, 18);
  const rows = [
    log('home-1', atLocal(2026, 7, 1)),
    log('external-mid', atLocal(2026, 7, 2), { source: 'external', beanId: null, cafeName: 'M' }),
    log('home-2', atLocal(2026, 7, 3))
  ];
  // 7/2 只有外饮，自家冲煮在 7/1 与 7/3 断开，连续天数应为 1 而不是 3。
  assert.equal(insights.coffeeCatalog(rows, [bean('bean-a')], { now }).data.milestones.streak.value, 1);
  // 反过来，外饮自己的连续天数也不该被自家冲煮续上。
  const flipped = [
    log('external-1', atLocal(2026, 7, 1), { source: 'external', beanId: null, cafeName: 'M' }),
    log('home-mid', atLocal(2026, 7, 2)),
    log('external-2', atLocal(2026, 7, 3), { source: 'external', beanId: null, cafeName: 'M' })
  ];
  assert.equal(insights.externalCatalog(flipped, { now }).data.milestones.streak.value, 1);
});

function extLog(id, consumedAt, extra) {
  return { id, consumedAt, source: 'external', beanId: null, cafeName: 'Seesaw', drinkName: '拿铁', location: '上海', price: 30, photos: [], ...extra };
}

test('外饮图鉴 0 杯外饮时使用独立 required=1 门槛，自家冲煮不算数', () => {
  const now = new Date(2026, 6, 15);
  const result = insights.externalCatalog([log('home-1', atLocal(2026, 7, 1))], { now });
  assert.equal(result.ok, false);
  assert.equal(result.reason, 'emptyExternal');
  assert.deepEqual(result.meta, { sampleSize: 0, required: 1, excludedCount: 0 });
});

test('咖啡馆收集墙忽略大小写合并同店：杯数累加、去过次数按天去重、封面取最近有图的一次', () => {
  const now = new Date(2026, 6, 15);
  const rows = [
    extLog('a', atLocal(2026, 7, 1), { cafeName: 'Seesaw', photos: ['old.jpg'] }),
    extLog('b', atLocal(2026, 7, 1, 18), { cafeName: 'seesaw', photos: [] }),
    extLog('c', atLocal(2026, 7, 4), { cafeName: 'SEESAW', photos: ['new.jpg'] }),
    extLog('d', atLocal(2026, 7, 5), { cafeName: 'Manner', drinkName: '美式' })
  ];
  const data = insights.externalCatalog(rows, { now }).data;
  assert.equal(data.cafes.items.length, 2);
  const seesaw = data.cafes.items[0];
  assert.equal(seesaw.cups, 3);
  // 7/1 记了两杯，只算去过一次。
  assert.equal(seesaw.visits, 2);
  assert.equal(seesaw.cover.candidates[0].path, 'new.jpg');
  assert.equal(seesaw.topDrink, '拿铁');
});

test('外饮收集墙收齐同店铺全部照片，最近的排在前面并去重', () => {
  const now = new Date(2026, 6, 15);
  const rows = [
    extLog('a', atLocal(2026, 7, 1), { cafeName: 'Seesaw', photos: ['old.jpg', 'dup.jpg'] }),
    extLog('b', atLocal(2026, 7, 4), { cafeName: 'seesaw', photos: ['new.jpg', 'dup.jpg'] }),
    extLog('c', atLocal(2026, 7, 5), { cafeName: 'Manner' })
  ];
  const items = insights.externalCatalog(rows, { now }).data.cafes.items;
  // 店名大小写不同视为同一家，格子沿用最近一次记录的写法。
  const seesaw = items.find((item) => item.name === 'seesaw');
  // 封面仍只取最近一张，但整店的照片都留了下来供炸开与轮播使用。
  assert.equal(seesaw.cover.candidates[0].path, 'new.jpg');
  assert.deepEqual(seesaw.photos, ['new.jpg', 'dup.jpg', 'old.jpg']);
  // 没拍照的店铺给空数组，渲染层据此判断不可点。
  assert.deepEqual(items.find((item) => item.name === 'Manner').photos, []);
});

test('冲煮收集墙把同款豆各次购买的饮用照片合并，最近的排在前面', () => {
  const now = new Date(2026, 6, 15);
  const beans = [
    bean('bean-old', { name: '耶加雪菲', roaster: 'Roaster' }),
    bean('bean-new', { name: '耶加雪菲', roaster: 'Roaster' }),
    bean('bean-solo', { name: '曼特宁', roaster: 'Roaster' })
  ];
  const rows = [
    log('r1', atLocal(2026, 7, 1), { beanId: 'bean-old', photos: ['old.jpg'] }),
    log('r2', atLocal(2026, 7, 6), { beanId: 'bean-new', photos: ['new.jpg', 'old.jpg'] }),
    log('r3', atLocal(2026, 7, 3), { beanId: 'bean-solo' })
  ];
  const wall = insights.coffeeCatalog(rows, beans, { now }).data.wall;
  const merged = wall.find((item) => item.name === '耶加雪菲');
  assert.equal(merged.purchaseCount, 2);
  assert.deepEqual(merged.photos, ['new.jpg', 'old.jpg']);
  // 只收饮用照片：没拍过照的豆子给空数组，不会因为有豆袋照就变成可点。
  assert.deepEqual(wall.find((item) => item.name === '曼特宁').photos, []);
});

test('单格照片上限截断，避免一格塞下无限张', () => {
  const now = new Date(2026, 6, 15);
  const rows = Array.from({ length: 8 }, (_, index) => extLog(`e${index}`, atLocal(2026, 7, index + 1), {
    cafeName: 'Manner',
    photos: [`p${index}-a.jpg`, `p${index}-b.jpg`]
  }));
  const cafe = insights.externalCatalog(rows, { now }).data.cafes.items[0];
  assert.equal(cafe.photos.length, insights.CATALOG_PHOTO_LIMIT);
});

test('sampleCatalogPhotos 按注入的 rng 取样并守住边界', () => {
  const photos = ['a.jpg', 'b.jpg', 'c.jpg', 'd.jpg'];
  // rng 恒为 0：Fisher-Yates 每次都和下标 0 交换，结果确定可断言。
  assert.deepEqual(insights.sampleCatalogPhotos(photos, 2, () => 0), ['b.jpg', 'c.jpg']);
  assert.equal(insights.sampleCatalogPhotos(photos, 3, () => 0).length, 3);
  // count 超过数量时返回全部（顺序打乱），不补空位。
  assert.equal(insights.sampleCatalogPhotos(photos, 99, () => 0).length, 4);
  assert.deepEqual(insights.sampleCatalogPhotos([], 3, () => 0), []);
  assert.deepEqual(insights.sampleCatalogPhotos(null, 3, () => 0), []);
  assert.deepEqual(insights.sampleCatalogPhotos(photos, 0, () => 0), []);
  assert.deepEqual(insights.sampleCatalogPhotos(['a.jpg', '', null], 3, () => 0).sort(), ['a.jpg']);
  // 默认 rng 也能跑，且取出的每张都来自原清单。
  insights.sampleCatalogPhotos(photos, 2).forEach((path) => assert.ok(photos.includes(path)));
});

test('收集墙一行装得下就排单行，超过阈值才回到两行', () => {
  const now = new Date(2026, 6, 15);
  const max = insights.CATALOG_WALL_SINGLE_ROW_MAX;
  const beansOf = (count) => Array.from({ length: count }, (_, index) => bean(`bean-${index}`, { name: `豆${index}` }));
  assert.equal(insights.coffeeCatalog([], beansOf(1), { now }).data.singleRow, true);
  assert.equal(insights.coffeeCatalog([], beansOf(max), { now }).data.singleRow, true);
  assert.equal(insights.coffeeCatalog([], beansOf(max + 1), { now }).data.singleRow, false);

  const extOf = (count) => Array.from({ length: count }, (_, index) => extLog(`e${index}`, atLocal(2026, 7, index + 1), { cafeName: `店${index}` }));
  assert.equal(insights.externalCatalog(extOf(max), { now }).data.singleRow, true);
  assert.equal(insights.externalCatalog(extOf(max + 1), { now }).data.singleRow, false);
});

test('外饮店名留空归入未记名格子并排在最后，杯数不丢失', () => {
  const now = new Date(2026, 6, 15);
  const rows = [
    extLog('a', atLocal(2026, 7, 1), { cafeName: '' }),
    extLog('b', atLocal(2026, 7, 2), { cafeName: 'Manner' }),
    extLog('c', atLocal(2026, 7, 3), { cafeName: 'Manner' })
  ];
  const data = insights.externalCatalog(rows, { now }).data;
  const unnamed = data.cafes.items[data.cafes.items.length - 1];
  assert.equal(unnamed.name, insights.CATALOG_UNNAMED_CAFE);
  assert.equal(unnamed.named, false);
  assert.equal(unnamed.cups, 1);
  // 未记名不计入「去过几家咖啡馆」的里程碑，但杯数照算。
  assert.equal(data.cafes.milestone.value, 1);
  assert.equal(data.milestones.cups.value, 3);
  assert.match(data.summary, /去过 1 家咖啡馆 · 喝过 3 杯/);
});

test('地点足迹按首次到访排序并统计店数，空地点跳过', () => {
  const now = new Date(2026, 6, 15);
  const rows = [
    extLog('a', atLocal(2026, 7, 5), { location: '北京', cafeName: 'Manner' }),
    extLog('b', atLocal(2026, 7, 1), { location: '上海', cafeName: 'Seesaw' }),
    extLog('c', atLocal(2026, 7, 2), { location: '上海', cafeName: 'Manner' }),
    extLog('d', atLocal(2026, 7, 3), { location: '' })
  ];
  const places = insights.externalCatalog(rows, { now }).data.places;
  assert.deepEqual(places.items.map((item) => item.name), ['上海', '北京']);
  assert.equal(places.items[0].cafeCount, 2);
  assert.equal(places.items[0].cups, 2);
  assert.equal(places.milestone.value, 2);
});

test('外饮花费只累计能估算的价格并给出未记价杯数', () => {
  const now = new Date(2026, 6, 15);
  const rows = [
    extLog('a', atLocal(2026, 7, 1), { price: 38 }),
    extLog('b', atLocal(2026, 7, 2), { price: 15.5 }),
    extLog('c', atLocal(2026, 7, 3), { price: 0 }),
    extLog('d', atLocal(2026, 7, 4), { price: null })
  ];
  const data = insights.externalCatalog(rows, { now }).data;
  assert.equal(data.spend.total, 53.5);
  assert.equal(data.spend.unknownCostCount, 2);
});

test('外饮图鉴分享 payload 换用咖啡馆文案并限制封面数量', () => {
  const now = new Date(2026, 6, 15);
  const rows = Array.from({ length: 10 }, (_, index) => extLog(`e${index}`, atLocal(2026, 7, 1 + index), { cafeName: `店${index}`, location: '' }));
  const data = insights.externalCatalog(rows, { now }).data;
  const payload = insights.buildExternalCatalogSharePayload(data, { coverLimit: 6 });
  assert.equal(payload.type, 'externalCatalog');
  assert.equal(payload.style, 'catalog');
  assert.equal(payload.wallLabel, '咖啡馆收集墙');
  assert.equal(payload.covers.length, 6);
  assert.equal(payload.remainingCovers, 4);
  // 外饮没有产地，收集墙副标题位改放常点饮品。
  assert.equal(payload.covers[0].origin, '拿铁');
  // 地点全空时不画地点 chips。
  assert.deepEqual(payload.origins, []);
  assert.equal(payload.milestones[1].label, '累计花费');
});
...
test('咖啡图鉴 UI 走个人中心直达、二级路由退回个人中心与分享 renderer', () => {
  ...
  const html = fs.readFileSync(path.join(__dirname, '..', 'www', 'index.html'), 'utf8');
  // 图鉴不再作为回顾首页的第 06 类 section。
  assert.doesNotMatch(insightsSource, /insightsSectionCatalog|catalog-home-section|<span>06<\/span>/);
  // 图鉴从个人中心直达，返回退回个人中心。
  assert.match(insightsSource, /function openCatalog\(tab\)[\s\S]*?state\.insightsExitTo = 'personal'/);
  assert.match(insightsSource, /if \(state\.insightsPage === 'reports' \|\| state\.insightsPage === 'catalog'\)[\s\S]*?exitInsights\(\)/);
  assert.match(insightsSource, /data-insights-catalog-share/);
  assert.match(appSource, /reopenPersonal: openPersonal/);
...
test('图鉴页内自家/外饮分栏切换：两栏各自渲染、分享按当前栏走对应 payload', () => {
  const appSource = fs.readFileSync(path.join(__dirname, '..', 'www', 'app.js'), 'utf8');
  const insightsSource = fs.readFileSync(path.join(__dirname, '..', 'www', 'app-insights.js'), 'utf8');
  const html = fs.readFileSync(path.join(__dirname, '..', 'www', 'index.html'), 'utf8');
  assert.match(html, /id="catalogTabs"/);
  assert.match(html, /data-insights-catalog-tab="home"/);
  assert.match(html, /data-insights-catalog-tab="external"/);
  // 两栏各有独立 renderer，由 renderCatalogPage 按 tab 分发。
  assert.match(insightsSource, /function renderHomeCatalog\(\)/);
  assert.match(insightsSource, /function renderExternalCatalog\(\)/);
  assert.match(insightsSource, /if \(tab === 'external'\) renderExternalCatalog\(\);\s*else renderHomeCatalog\(\);/);
  // 外饮栏走 externalCatalog，不传 beans。
  assert.match(insightsSource, /core\.externalCatalog\(state\.drinkLogs, \{ photoJournal/);
  assert.match(insightsSource, /shareCatalog\(result\.data, \{ external \}\)/);
  assert.match(appSource, /options && options\.external \? BeanInsights\.buildExternalCatalogSharePayload/);
  assert.match(appSource, /insightsCatalogTab: 'home'/);
});
