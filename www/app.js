(function () {
  ...
  function openCoffeeCalendar(view) { state.coffeeCalendarView = view || state.coffeeCalendarView || 'month'; if (!state.selectedCoffeeDay) state.selectedCoffeeDay = ymd(new Date()); setDialog(els.personal, false); renderCoffeeCalendar(); setDialog(els.calendar, true); }
  ...
  function planToForm(plan) {
    ...
    renderPourSteps(p.steps); renderPlanBeanBind(p.beanIds); syncPlanMethodFields(); syncDurationControls(els.planForm); syncPlanTotalWater(); syncAllChoiceTriggers();
  }
  ...
  async function importText(text) { const imported = BeanCore.validateImport(JSON.parse(text)); if (!confirm(`备份包含 ${imported.beans.length} 款豆子、${imported.drinkLogs.length} 杯记录、${imported.brewPlans.length} 个方案，将覆盖当前数据。继续吗？`)) return; await BeanRepository.replaceAllData(imported.beans, imported.drinkLogs, imported.settings, imported.brewPlans); await reload(); setDialog(els.settings, false); setDialog(els.personal, false); toast(`已恢复 ${imported.beans.length} 款豆子、${imported.drinkLogs.length} 杯记录与 ${imported.brewPlans.length} 个方案`); }
  ...
  function bindEvents() {
    ...
    $('#brewPlanList').addEventListener('click', (event) => { const card = event.target.closest('[data-plan-id]'); if (card) openPlanDetail(state.brewPlans.find((plan) => plan.id === card.dataset.planId)); });
