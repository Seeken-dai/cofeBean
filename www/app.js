(function () {
  ...
  const insightsUi = window.AppInsights.create({ state, dialog: els.insights, core: BeanInsights, setDialog, toast, shareReport: shareCoffeeReport });
  ...
  function bindEvents() {
    ...
    $('#drink-cafeName').addEventListener('input', renderCafeSuggestions);
    $('#drinkCafeSuggestions').addEventListener('click', (event) => { const choice = event.target.closest('[data-cafe-name]'); if (!choice) return; $('#drink-cafeName').value = choice.dataset.cafeName; renderCafeSuggestions(); $('#drink-cafeName').focus(); });
