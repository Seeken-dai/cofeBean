(function () {
  ...
  async function importText(text) { const imported = BeanCore.validateImport(JSON.parse(text)); const scope = backupScope(imported.exportScope); const summary = backupSummary(scope, imported); const mode = chooseImportMode(scope, summary); if (!mode) return; await BeanRepository.importData(imported, mode); await reload(); setDialog(els.backup, false); setDialog(els.settings, false); setDialog(els.personal, false); toast(`${mode === 'merge' ? '已合并' : '已导入'}${backupScopes[scope].title}`); }
