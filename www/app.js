(function () {
  ...
  function applyTheme(theme, persist) { if (!themeColors[theme]) theme = 'dark-roast'; document.documentElement.dataset.theme = theme; state.settings.theme = theme; localStorage.setItem('coffee-vault-theme', theme); $$('[data-theme-value]').forEach((button) => button.classList.toggle('active', button.dataset.themeValue === theme)); const statusBar = capPlugin('StatusBar'); if (statusBar) { if (statusBar.setOverlaysWebView) statusBar.setOverlaysWebView({ overlay: false }).catch(() => {}); statusBar.setBackgroundColor({ color: themeColors[theme] }).catch(() => {}); statusBar.setStyle({ style: theme === 'frost' ? 'LIGHT' : 'DARK' }).catch(() => {}); } if (persist && state.initialized) BeanRepository.saveSettings(state.settings).catch(() => toast('主题保存失败')); }
  ...
  function bindEvents() {
    ...
    $('#planDetailClose').addEventListener('click', () => setDialog(els.planDetail, false)); $('#planShare').addEventListener('click', sharePlanCard); $('#planAssistStart').addEventListener('click', openPlanBrewAssist);
